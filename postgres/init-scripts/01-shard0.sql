-- postgres/init-scripts/02-shard0-schema.sql
-- Complete initialization script for shard0 database (shard_key = 0)
...
CREATE TABLE IF NOT EXISTS users (
    ...
    shard_key INT NOT NULL DEFAULT 0 CHECK (shard_key = 0),
...
CREATE TABLE IF NOT EXISTS messages (
    ...
    shard_key INTEGER NOT NULL DEFAULT 0 CHECK (shard_key = 0),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id, shard_key)
) PARTITION BY LIST(shard_key);

-- Create partition for shard0
...
CREATE INDEX IF NOT EXISTS idx_messages_sender ON messages(sender_id) WHERE shard_key = 0;
CREATE INDEX IF NOT EXISTS idx_messages_receiver ON messages(receiver_id) WHERE shard_key = 0;
